...
service cloud.firestore {
	match /databases/{database}/documents {
		...
		match /tags/{tagId} {
			// --- THE NEW, COMBINED RULE ---
			// Allow a read if EITHER of these is true:
			// 1. (For collection queries): The document's 'userId' field matches the user's UID.
			// 2. (For transaction 'get's): The user's UID is in the document's ID.
			allow read: if request.auth != null &&
				(resource.data.userId == request.auth.uid || tagId.split('_')[0] == request.auth.uid);

			// It's safer to keep the delete rule specific to the document ID.
			allow delete: if request.auth != null
				&& tagId.split('_')[0] == request.auth.uid;

			// This rule remains correct, validating the data being written.
